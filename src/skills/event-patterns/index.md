...
Publishers Zod-validate and emit events to the `myorg.domain` exchange.
Subscribers bind to routing key patterns and Zod-parse incoming messages.
Failed messages route to a Dead Letter Queue (`myorg.dlx`). Correlation IDs
propagate through the entire event chain for distributed tracing.

```
┌─────────────┐     ┌──────────────┐     ┌─────────────────────┐
│  Service A   │     │  Publisher    │     │  RabbitMQ           │
│  (business   │────▶│  Zod validate│────▶│  myorg.domain      │
...
| 4   | [rmq-client.md](rmq-client.md) | CustomRmqClient — topic exchange via channel.publish        |
