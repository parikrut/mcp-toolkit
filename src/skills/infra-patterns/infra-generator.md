...
1. **`apps/<product>-web/nginx.conf`** — A complete nginx reverse-proxy configuration that routes each API path prefix to the correct backend service, serves the SPA with history-mode fallback, and applies security headers + gzip compression.

2. **`products/<product>/infra/init-databases.sql`** — SQL script that creates one database per service (DB-per-service pattern) and grants privileges to the shared database user. Mounted into the PostgreSQL container via Docker Compose.

3. **`products/<product>/start-dev.sh`** — An executable shell script that starts the entire development stack with one command: runs `docker compose up`, polls health endpoints until all services are ready, and prints a summary of URLs (frontend, Swagger docs, infrastructure UIs).
...
All generated files include a header: `AUTO-GENERATED by infra/generate.ts — do not edit manually.` with a reference to the service registry as the source of truth. These files should be committed to git so they're available without running the generator, but must never be manually edited.
...
7. **Generated files have headers.** Every output file starts with a comment block: `AUTO-GENERATED by infra/generate.ts — do not edit manually.`
8. **Nginx routes are grouped by layer.** Platform routes first, then shared, then domain — matching the registry's layer annotations.
9. **SQL creates databases for services with non-empty `database` field.** Services with `database: ""` (e.g., API gateway) are skipped.
10. **Start-dev script uses health polling.** Each service is polled on its `/health` endpoint with a timeout of 60 iterations × 2 seconds. Infrastructure services are polled first.
...
├── infra/
│   ├── service-registry.ts     # Input  — typed service definitions
│   ├── generate.ts             # This script — reads registry, writes 3 files
│   └── init-databases.sql      # Output — auto-generated SQL
├── start-dev.sh                # Output — auto-generated dev startup script
└── ...

apps/<product-name>-web/
└── nginx.conf                  # Output — auto-generated nginx config
```
 ...
 * Validates:
 *   • No duplicate ports
 *   • No duplicate API route prefixes
 *   • All extraEnv service URLs reference valid registry entries
 */
...
function validate(): void {
    ...
    console.log(
        `✅ Validated ${ports.size} services, ${routes.size} routes — no conflicts`,
    );
}
...
function generateNginxConf(): string {
    ...
    for (const layer of layerOrder) {
        ...
        for (const svc of services) {
            locationBlocks.push(`    # ${svc.label} — :${svc.port}`);
            for (const route of svc.routes) {
                locationBlocks.push(
    ...
    return `# ──────────────────────────────────────────────────────────────
# Product Web — Nginx reverse-proxy / SPA host
# ──────────────────────────────────────────────────────────────
...
server {
    ...
    # ── API proxying (auto-generated from service registry) ─
...
// ─── Start-Dev Shell Script Generator ────────────────────────
...
echo "⚙️  Generating infrastructure config from service registry..."
...
for i in {1..30}; do
    ...
    if [[ "${$}POSTGRES_OK" -ge 1 && "${$}RABBITMQ_OK" -ge 1 && "${$}REDIS_OK" -ge 1 ]]; then
        echo "  ✔ Infrastructure ready (PostgreSQL, RabbitMQ, Redis)"
        break
    fi
    sleep 2
done
...
console.log(\`📝 Generated \${path.relative(ROOT, NGINX_OUT)}\`);
...
console.log(\`📝 Generated \${path.relative(ROOT, DEV_SH_OUT)}\`);
...
**`nginx.conf` — generated route block:**
...
4. The generator functions are product-agnostic — only the import path and output paths change.
