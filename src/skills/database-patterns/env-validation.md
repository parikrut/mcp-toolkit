...
The validation engine is Zod. A `BaseEnvSchema` is defined in the `@myorg/common` package and includes all environment variables shared across services: `PORT`, `NODE_ENV`, `DATABASE_URL`, `RABBITMQ_URL`, `JWT_SECRET` / `JWT_ACCESS_SECRET`, `JWT_EXPIRES_IN`, `CORS_ORIGIN`, `LOG_LEVEL`, and `RATE_LIMIT_MAX`. Numeric fields use `z.coerce.number()` so they are automatically parsed from string environment variables. Fields with sensible defaults (like `PORT: 3000`, `NODE_ENV: "development"`) use `.default()` so they are optional in the environment but always present in the typed output.

Each module extends `BaseEnvSchema` with module-specific variables using Zod's `.extend()` method. For example, the billing module adds `ORDER_MANAGEMENT_URL`, the payment module adds `ENCRYPTION_KEY`, and so on. The extended schema is exported as a named type (`ModuleEnv`) for use throughout the module.
...
1. **`BaseEnvSchema` is the foundation.** Every module's env schema must extend `BaseEnvSchema` from `@myorg/common`. Never define shared fields (PORT, NODE_ENV, JWT_SECRET, etc.) independently in a module.
...
11. **URL validation for service URLs.** Module-specific service URLs (e.g., `ORDER_MANAGEMENT_URL`) should use `z.string().url()` to validate they are properly formatted URLs.
...
| ------------------------------ | --------------- |
| `z`                            | `zod/v4`        |
| `BaseEnvSchema`, `validateEnv` | `@myorg/common` |
...
**Module-specific schema (`modules/domain/revenue/billing/src/env.ts`):**

```typescript
import { z } from "zod/v4";
import { BaseEnvSchema, validateEnv } from "@myorg/common";

/**
 * Billing & Instalment — Environment Schema
 *
 ...
 *   - ORDER_MANAGEMENT_URL: HTTP client URL for cross-service data
...
const BillingEnvSchema = BaseEnvSchema.extend({
    ...
    // ── Cross-service URLs ──────────────────────────────────
    ORDER_MANAGEMENT_URL: z.string().url("ORDER_MANAGEMENT_URL must be a valid URL"),
...
export const env = validateEnv(BillingEnvSchema, process.env);
```

**Module entrypoint using validated env (`modules/domain/revenue/billing/src/main.ts`):**
...
async function main() {
    ...
    const app = await bootstrapModule(BillingModule, {
        port: env.PORT,
        cors: env.CORS_ORIGIN,
        logLevel: env.LOG_LEVEL,
    });

    console.log(`Billing & Instalment service running on port ${env.PORT} [${env.NODE_ENV}]`);
}

main();
```

**Payment module with encryption key requirement:**
...
export type PaymentServiceEnv = z.infer<typeof PaymentServiceEnvSchema>;

export const env = validateEnv(PaymentServiceEnvSchema, process.env);
```
...
Error: Environment validation failed:
  ...
  - ORDER_MANAGEMENT_URL: ORDER_MANAGEMENT_URL must be a valid URL
